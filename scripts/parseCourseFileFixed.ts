...
function parseCourseFile(filePath: string): ParsedCourse {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const originalLine = lines[i]; // Keep original for text content
...
async function deleteCourse(courseId: string) {
  ...
  // Prisma will cascade delete all related modules, topics, and classes
...
async function main() {
  try {
    ...
    const oldCourseId = 'cmj864bum0000sgk8grr0jhmc';
    try {
      await deleteCourse(oldCourseId);
    } catch (e) {
      console.log('No old course to delete, continuing...');
    }
