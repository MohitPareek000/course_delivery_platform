 ...
 *
 * This script updates course content WITHOUT losing user progress.
 * It matches existing content by module order, topic order, and class order,
 * then updates content in place rather than deleting and recreating.
 *
 * Key benefits:
 * - User progress is preserved (class IDs stay the same)
 * - Only changed content is updated
 * - New content is added, removed content stays (safe)
 */
...
async function updateCourseIncremental(parsedCourse: ParsedCourse, courseId: string) {
  ...
  // Create a map of existing modules by order
  const existingModulesMap = new Map(
    course.modules.map(m => [m.order, m])
  );

  // Process each module from parsed content
  for (const moduleData of parsedCourse.modules) {
    ...
    // Create a map of existing topics by order
    ...
    for (const topicData of moduleData.topics) {
      ...
      // Create a map of existing classes by order
      ...
      for (const classData of topicData.classes) {
        ...
        if (existingClass) {
          // Update existing class - PRESERVE THE ID!
  ...
  console.log('   (Class IDs remain unchanged for existing content)');
