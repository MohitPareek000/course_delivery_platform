...
async function main() {
  try {
    // Find the "Leadership and Teamwork Analysis" class
    const leadershipClass = await prisma.class.findFirst({
      where: {
        title: {
          contains: 'Leadership and Teamwork',
        },
      },
      include: {
        topic: {
    ...
    if (!leadershipClass) {
      console.error('❌ Could not find "Leadership and Teamwork Analysis" class');
      return;
    }

    console.log('\n📍 Found class:');
    console.log(`   Course: ${leadershipClass.topic.module?.course.title}`);
    console.log(`   Module: ${leadershipClass.topic.module?.title}`);
    console.log(`   Topic: ${leadershipClass.topic.title}`);
    console.log(`   Class: ${leadershipClass.title}`);

    // Update the class
    await prisma.class.update({
      where: { id: leadershipClass.id },
      data: {
    ...
    console.log(`   "${leadershipClass.title}" → "Navigating the Maze of Data Role Titles"`);
